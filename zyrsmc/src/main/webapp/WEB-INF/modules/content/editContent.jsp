...
<body id="index">
    ...
    <div class="w-content m-center">
       <form action="${ctx }/content/saveContent" method="post" id="inputForm">
        <input type="hidden" name="id" value="${content.id }"/>
         <div class="content">
             <div class="control">
               <label>标题:</label>
               <span class="inp"><input class="width300" type="text" value="${content.title }" name="title"/></span>
             </div>
             <div class="control">
               <label>编辑:</label>
               <span class="inp"><input class="width300" type="text" value="${content.editor }" name="editor"/></span>
             </div>
             <div class="control">
               <label>摘要:</label>
               <span class="inp"><input class="width300" type="text" value="${content.summary }" name="summary"/></span>
             </div>
             <div class="control">
               <label>所属栏目:</label>
               <span class="inp">
                   ...
                   <option value="">请选择栏目</option>
                     <c:forEach items="${columnList}" var="column">
                       <c:if test="${column.parentId ==0 }">
	                       <option value="${column.id}" ${column.id eq content.columnId?'selected=selected':'' }>
	                       ${column.columnName}</option>
	                       <c:forEach items="${columnList}" var="scolumn">
		                       <c:if test="${scolumn.parentId ==column.id }">
			                       <option value="${scolumn.id}" ${scolumn.id eq content.columnId?'selected=selected':'' }>
			                       &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;${scolumn.columnName}</option>
		                       </c:if>
		                     </c:forEach>
                       </c:if>
                     </c:forEach>
             ...
             <div class="control">
               ...
               <img src="${ctx}${content.picture }" style="width:100px;"/>
               </span>
             </div>
             <div class="control">
               <label style="float:left;">内容:</label>
               <span class="inp"><textarea id="content" rows="10" cols="100" name="content"  style="display:none;">${content.content }</textarea>
                <script id="editor" type="text/plain" style="width:96%;min-height:200px;margin-left:30px;"></script>
               </span>
             </div>
             <div class="control">
               <label>排序:</label>
               <span class="inp"><input class="width300" type="number" value="${content.orderFlag }" name="orderFlag"/></span>
             </div>
             <div class="control">
               <label>点击量:</label>
               <span class="inp"><input class="width300" type="number" value="${content.clickNum }" name="clickNum"/></span>
             </div>
             <div class="operation">
